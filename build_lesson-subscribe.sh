...
ROOT_DIR="pod/subscribe"
# Get the original directory
ORIGINAL_DIR=$(pwd)
# Define the output file in the original directory
output_file="$ORIGINAL_DIR/lesson-02C-distributed-subscribe.md"
# Define the README path in the original directory
README_PATH="$ORIGINAL_DIR/README_SUBSCRIBE.md"
...
get_directory_tree() {
    ...
    while [[ ${#stack[@]} -gt 0 ]]; do
        ...
        # Use find to list items, excluding 'target' and hidden directories
...
# Initialize output file, including README_SUBSCRIBE.md if it exists
...
{
    echo '# Lesson 02C: Distributed Subscribe'
    echo ''
...
for toml in "${cargo_tomls[@]}"; do
    ...
    for file in "${files[@]}"; do
        ...
        relative_path=$(realpath --relative-to="$ORIGINAL_DIR" "$absolute_file")
        ext="${file##*.}"
        # Add file header
        echo "## $relative_path" >> "$output_file"
        echo '' >> "$output_file"
        # Set code block delimiter based on extension
        case "$ext" in
            rs) echo '```rust' ;;
            toml) echo '```toml' ;;
            *) echo '```text' ;;
